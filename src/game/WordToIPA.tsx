...
export default function WordToIPA() {
  ...
  const handleAnswer = (userChoice: string) => {
    ...
    const easedIPA = currentQuestion.ipa.map(i => i.replaceAll(/(\.|ˈ|ː)/g, ""))

    console.log(easedIPA)
    if (currentQuestion.ipa.includes(userChoice) || easedIPA.includes(userChoice)) {
      handleCorrect()
    }
    else {
      handleIncorrect()
        ...
        <div id="result-input">
          {score.map((s, i) => (
            ...
            >
              ...
              <div className="result-ipa">{s.ipa.map(s => "/" + s + "/  ")}</div>
            </button>
          ))}
        </div>
      </>
    ...
    return (
      <>
        ...
        <div id="word-container" className={handleAnswerColor()}>
          ...
          <div id="word-body" className={handleAnswerColor()}>{ currentQuestion.word }</div>
        </div>
          {
